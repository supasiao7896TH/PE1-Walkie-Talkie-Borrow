...
- Project name: `pe1-walkie-talkie-borrow` → URL ที่ได้: `https://pe1-walkie-talkie-borrow.pages.dev`
  (`wrangler pages deploy` เองไม่สร้าง project ให้ — workflow มี step "Ensure Cloudflare Pages project exists"
  เช็คด้วย `wrangler pages project list` ก่อน สร้างเฉพาะตอนยังไม่มี ไม่ต้องสร้างเองผ่าน dashboard)
