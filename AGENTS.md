# AGENTS.md — PE1 Walkie Talkie Borrow Project Guide

คู่มือนี้สำหรับ AI coding agent ทุกตัว (Claude Code, Codex, Cursor, GitHub Copilot ฯลฯ) ที่เข้ามาทำงานในโปรเจกต์นี้
เพื่อให้เข้าใจโครงสร้างและขั้นตอนที่ถูกต้องโดยไม่ต้องอธิบายซ้ำ

ดูภาพรวมโปรเจกต์แบบสั้น/เชิงธุรกิจได้ที่ [`context.md`](./context.md) — ไฟล์นี้เน้นกฎและขั้นตอนทำงานเชิงลึกสำหรับ
agent โดยเฉพาะ (เนื้อหาเดียวกันกับ [`CLAUDE.md`](./CLAUDE.md) ซึ่งเขียนไว้สำหรับ Claude Code)
...
├── context.md          ← ภาพรวมโปรเจกต์แบบสั้น/เชิงธุรกิจ
├── AGENTS.md           ← ไฟล์นี้ (คู่มือสำหรับ AI agent ทุกตัว)
├── CLAUDE.md            ← เนื้อหาเดียวกันกับ AGENTS.md แต่เขียนสำหรับ Claude Code
├── .github/
│   └── workflows/
...
4. git add, git commit, git push → branch: feature branch ตาม convention ของทีม
...
Banner HTML: `#line-banner` ใน `index.html`
...
- ทีมงานและผู้ใช้เป็นคนไทย — สื่อสาร/อธิบายเป็นภาษาไทย
